...
const BIN_ID = process.env.NEXT_PUBLIC_JSONBIN_BIN_ID || '676b8e8ead19ca34f8d4b8a7';
...
export class JSONBinStorage {
    ...
    async getBooks() {
        try {
            ...
            console.warn('Error fetching books from JSONBin:', error.message);
            return null; // Will fallback to localStorage
        }
    }
...
export async function testJSONBinAccess() {
    ...
    try {
        ...
        if (books !== null && Array.isArray(books)) {
            console.log('✅ JSONBin read access successful!', `Found ${books.length} books`);
            return true;
        } else {
            console.log('❌ JSONBin read access failed');
            return false;
        }
    } catch (error) {
...
// Simple test function for basic connectivity
